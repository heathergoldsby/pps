#ifndef _EALIFE_STRIPES_REPLICATION_H_
#define _EALIFE_STRIPES_REPLICATION_H_
    ...
    virtual void operator()(MEA& mea) {
        ...
        if ((mea.current_update() % ru) == 0) {
            ...
            for(typename MEA::iterator i=mea.begin(); i!=mea.end(); ++i) {
                ...
                if (get<MULTICELL_REP_TIME>(*i, 0) >= get<JUV_PERIOD>(*i,0)) {
...
//! Performs multicell replication using germ lines.
template <typename MEA>
struct stripes_replication_evo_plane_no_parent_reset : end_of_update_event<MEA> {
    //! Constructor.
    stripes_replication_evo_plane_no_parent_reset(MEA& mea) : end_of_update_event<MEA>(mea), _df("stripes_replication_evo_plane.dat") {
        _df.add_field("update")
        .add_field("mean_rep_time")
        .add_field("last_fitness")
        .add_field("last_fitness_pt")
    ...
    //! Destructor.
    virtual ~stripes_replication_evo_plane_no_parent_reset() {
    }
