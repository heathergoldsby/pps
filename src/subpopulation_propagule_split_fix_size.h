 ...
 */
#ifndef _SUBPOPULATION_PROPAGULE_SPLIT_FIX_SIZE_H_
#define _SUBPOPULATION_PROPAGULE_SPLIT_FIX_SIZE_H_
...
struct subpopulation_propagule_split_fix_size {
    ...
    void operator()(Population& parents, Population& offspring, MEA& mea) {
        ...
        if(get<PROP_COUNT>(*(parents[0])) < num_prop) {
            num_prop = get<PROP_COUNT>(*(parents[0]));
        }
