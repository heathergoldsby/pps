 ...
 */
#ifndef _SUBPOPULATION_PROPAGULE2_H_
#define _SUBPOPULATION_PROPAGULE2_H_
...
/*! This recombination operator is metapopulation-specific; it copies a
 series of propagule cells from a parent subpopulation, and adds it to an
 offspring subpopulation. Differing from the original subpopulation_propagule
...
struct subpopulation_propagule {
    ...
    void operator()(Population& parents, Population& offspring, MEA& mea) {
        ...
        std::size_t prop_size = get<NUM_PROPAGULE_CELL>(mea,1);
        assert(prop_size > 0);
